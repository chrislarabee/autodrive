...
class TestGSheetView:
    def test_that_it_can_parse_row_data(self):
        ...
        assert GSheetView._parse_row_data(raw, value_type=UserEnteredVal) == expected
        expected = [[None, None, "test"], [None, 1, None], [3]]
        assert GSheetView._parse_row_data(raw, value_type=EffectiveVal) == expected
        expected = [[None, None, "test"], [None, "1", None], ["3"]]
        assert GSheetView._parse_row_data(raw, value_type=FormattedVal) == expected
